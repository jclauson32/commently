...
def test_llm_batch_benchmark():
    ...
    dataset_version = dataset.get("version", "No Version")
    ...
    assert len(batch_result.ratings) == len(
        dataset["benchmarks"]
    ...
    if "GITHUB_STEP_SUMMARY" in os.environ:
        with open(os.environ["GITHUB_STEP_SUMMARY"], "a") as summary_file:
            ...
            summary_file.write(f"- **Benchmark Version:** {dataset_version}\n")
            summary_file.write(
                f"- **Total Processed:** {batch_result.total_processed}\n"
            )
            summary_file.write(
